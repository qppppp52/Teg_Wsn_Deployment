...
def evaluate_init_individual(individual, ctx, config, source_type="unknown") -> dict:
    ...
    sol, rep_ind = evaluate_individual(individual, ctx, max_repair_iter=int(cfg.get("max_repair_iter", ctx.config.get("constraints", {}).get("max_repair_iter", 5))))
...
def quality_score(solution, config) -> float:
    ...
    rsum_norm = min(float(solution.metadata.get("throughput_actual", solution.throughput)) / rsum_ref, 1.0)
