...
def make_convergence_history() -> dict:
    ...
    keys = [
        ...
        "coverage_mean_feasible", "rsum_actual_best", "rsum_capacity_best",
        "saturated_link_ratio", "num_saturated_links", "mean_link_capacity_bps",
...
def record_generation(history: dict, population, archive, config: dict) -> dict:
    ...
    metric = {
        ...
        "Rsum_feasible_mbps": _mean([s.throughput for s in feasible], nan=True) / 1e6 if feasible else float("nan"),
        "Coverage_all": _mean([s.coverage for s in sols]),
        "Rsum_all_mbps": _mean([s.throughput for s in sols]) / 1e6,
        ...
        "rsum_actual_best": max([float(s.metadata.get("throughput_actual", s.throughput)) for s in feasible], default=0.0),
        "rsum_capacity_best": max([float(s.metadata.get("throughput_capacity", s.throughput)) for s in feasible], default=0.0),
        "saturated_link_ratio": _mean([s.metadata.get("saturated_link_ratio", 0.0) for s in sols]),
        "num_saturated_links": _mean([s.metadata.get("num_saturated_links", 0) for s in sols]),
