...
def _ctx(metric):
    return SimpleNamespace(
        ...
        config={
            ...
            "channel": {"sensor_data_rate_bps": 2.0e6, "use_data_rate_cap": True},
        },
    )


def test_evaluate_objectives_uses_actual_or_capacity(monkeypatch):
    import src.objectives.throughput_objective as throughput_module

    monkeypatch.setattr(throughput_module, "compute_snr", lambda *args, **kwargs: 1.0)
    monkeypatch.setattr(throughput_module, "compute_rate_components", lambda snr, config: (100.0, 10.0))
    sol = Solution(2)
    sol.x[0] = 1
    sol.y[1] = 1
    sol.c[0, 1] = 1
    evaluate_objectives(sol, _ctx("actual"))
    assert sol.throughput == 10.0
    evaluate_objectives(sol, _ctx("capacity"))
    assert sol.throughput == 100.0
...
def test_drl_reward_and_quality_score_use_selected_metric(monkeypatch):
    ...
    sol.cv = 0.0
    sol.feasible = True
    sol.metadata = {"throughput_actual": 10.0, "throughput_capacity": 90.0}
